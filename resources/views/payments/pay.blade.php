                                        ...
                                        <div style="position: relative; width: 100%; max-width: 450px; margin: 0 auto; padding: 25px; background: white; border-radius: 5px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
                                            ...
                                            <div style="text-align: center; margin-top: 18px; margin-bottom: 10px; padding-bottom: 10px; display: flex; justify-content: center; align-items: center; gap: 15px;">
                                                <div>
                                                    <img src="{{ asset('images/client.png')}}"
                                                        style="width: 120px; margin: 0 auto auto auto"
                                            ...
                                            <div class="oversized" style="display: flex; justify-content: space-between; margin: 5px 0 5px 0;">
                                                ...
                                                <div style="font-size: 20px; font-weight: 800; text-transform: uppercase">
                                                    {{ number_format(abs((float) $data['current_bill']['total'] - $prevUnpaid), 2) }}
                                                </div>
                                            </div>
                                            <div style="margin: 5px 0 5px 0; width: 100%; height: 1px; border-bottom: 1px dashed black;"></div>
                                            <div class="oversized" style="display: flex; justify-content: space-between; align-items: center;">
                                                ...
                                                <div style="text-transform: uppercase; font-size: 20px; font-weight: 800;">{{ number_format(abs((float) $data['current_bill']['total'] - (float) $totalDiscount + (float) $franchiseTax), 2) }} </div>
                                            </div>
                                            ...
                                            @php
                                                $penalty = $data['current_bill']['penalty'] ?? null;
                                                $discount = 0;
                                            ...
                                            <div class="oversized" style="margin: 5px 0 0 0; display: flex; justify-content: space-between; align-items: center;">
                                                ...
                                                <div style="text-transform: uppercase; font-size: 20px; font-weight: 800;">
                                                    {{number_format(abs((float) $data['current_bill']['amount'] - (float) $totalDiscount + (float) $tax), 2)}}
                                                </div>
                                            </div>
                        ...
                        <div class="col-12 col-md-6">
                            @if(!$data['current_bill']['isPaid'])
                                @php
                                    ...
                                    $advancePayment = (float)($data['current_bill']['advances'] ?? 0);
                                    $prevUnpaid = (float)($data['current_bill']['previous_unpaid'] ?? 0);
                                    $tax = (float)($data['current_bill']['tax'] ?? 0);
                                    $hasAdvancePayment = $data['current_bill']['isChangeForAdvancePayment'] ?? false;
                                    $netCurrentBill = max(0, $currentBill - $discount - $advancePayment - $penalty);
                                @endphp

                                <div class="bg-danger d-flex align-items-center justify-content-between mt-4 p-3 text-uppercase fw-bold text-white">
                                    Total Amount Due:
                                    <h3 class="ms-2">
                                        PHP {{number_format((float) $data['current_bill']['total'] - ($discount) - ($advancePayment) + ($applicablePenalty) + $tax ?? 0, 2)}}
                                    </h3>
                                </div>
                                <div class="card mt-4">
                                    <div class="card-body">
                                        ...
                                        @php
                                            ...
                                            $totalDue = $netCurrentBill + $applicablePenalty + $tax;
                                        @endphp
                                        ...
                                        <div class="mb-3">
                                            <div class="text-end">
                                                <label for="total_charges" class="form-label">Current Billing</label>
                                                 @php
                                                    $current_billing = (float)$data['current_bill']['total'] - (float) $data['current_bill']['previous_unpaid'];
                                                    $total_billing = (float)$data['current_bill']['amount'];
                                            ...
                                            @if($tax > 0)
                                                <div class="text-end">
                                                    <h6 class="text-danger" style="font-size: 12px;">+ PHP {{ number_format($tax, 2) }} ( TAX )</h6>
                                                </div>
                                            @endif
